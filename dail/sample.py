...
def eps_greedy_sample(logits, eps):
    ...
    rand_num = tf.random_uniform(
        [tf.shape(logits)[0], 1], minval=0, maxval=1, dtype=tf.float32
    )
    mask = tf.cast(rand_num < eps, tf.int32)

    rand_sample = tf.random_uniform(
        [tf.shape(logits)[0], 1], minval=0, maxval=tf.shape(logits)[1], dtype=tf.int32
    )
...
if __name__ == "__main__":
    logits = tf.log(tf.constant([[1.0], [1.0]]))
    eps = tf.placeholder(tf.float32, ())

    with tf.Session() as sess:
        sample = sess.run(eps_greedy_sample(logits, eps), feed_dict={eps: 0.0})
        print(sample)
