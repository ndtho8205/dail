...
def ddpg_graph_with_goal(env, ph, params):
    ...
    if "reacher2" in env["expert"]["name"]:
        ...
        print(
            "[ddpg_graph_with_goal.py] ERROR: unrecognized expert env name {}".format(
                env["expert"]["name"]
            )
        )
    ...
    if "reacher2" in env["learner"]["name"]:
        ...
        print(
            "[ddpg_graph_with_goal.py] ERROR: unrecognized learner env name {}".format(
                env["learner"]["name"]
            )
        )
    ...
    for d_ in sorted(env.keys()):
        ...
        # Self policy
            ...
            with tf.variable_scope("", reuse=tf.AUTO_REUSE):
                ...
                graph[d_]["inv_agent_state"] = feedforward(
                    in_node=graph[d_]["mapped_agent_state"],
                    is_training=ph[d_]["is_training"],
                ...
                expert_agent_state = tf.concat(
                    [
                        ph[trans_d_]["state"][:, : 2 * EXP_NJOINTS],
                        ph[trans_d_]["state"][:, 2 * EXP_NJOINTS + 2 :],
                    ],
                    axis=1,
                )
                expert_goal = (
                    ph[trans_d_]["state"][:, 2 * EXP_NJOINTS : 2 * EXP_NJOINTS + 2]
                    * GOAL_SCALE
                )

                mapped_lea_agent_state = feedforward(
                    in_node=expert_agent_state,
                    is_training=ph[d_]["is_training"],
                    params=params[d_]["invmap"],
                    scope="actor/" + d_ + "/invmap",
                    scale=params["train"]["scale_state"],
                ...
                graph[trans_d_]["mapped_state"] = tf.concat(
                    [
                ...
                graph[trans_d_]["mapped_action"] = feedforward(
                    in_node=ph[trans_d_]["action"],
                    is_training=ph[d_]["is_training"],
                    params=params[d_]["actionmap"],
                    scope="actor/" + d_ + "/actionmap",
                    scale=True,
    ...
    with tf.variable_scope("discriminator", reuse=tf.AUTO_REUSE):
        ...
        sas_real = tf.concat(
            [ph[trans_d_]["state"], ph[trans_d_]["action"], ph[trans_d_]["next_state"]],
            axis=1,
        )
    ...
    for d_ in env.keys():
        ...
        if d_ == "expert":
            ...
            graph_vars[d_]["all_slow_actor_vars"] = tf.get_collection(
                glob_vars, scope="slow_target_actor/" + d_
            )

        else:
            graph_vars[d_]["actor_grad_vars"] = tf.get_collection(
                glob_vars, scope="actor/" + d_ + "/statemap"
            )
            graph_vars[d_]["actor_grad_vars"] += tf.get_collection(
                glob_vars, scope="actor/" + d_ + "/actionmap"
            )

            # For inverse statemap
            graph_vars[d_]["auto_grad_vars"] = tf.get_collection(
                glob_vars, scope="actor/" + d_ + "/invmap"
            )
        ...
        graph_vars[d_]["model_grad_vars"] = tf.get_collection(
            glob_vars, scope="model/" + d_
        )

        # Discriminator vars
        graph_vars[d_]["disc_grad_vars"] = tf.get_collection(
            glob_vars, scope="discriminator/" + d_
        )
    ...
    for d_ in env.keys():
        ...
        if d_ == "learner" and params[d_]["use_bc"]:
            action_loss = tf.reduce_mean(
                tf.square(graph[d_]["action"] - ph[d_]["raw_action"])
            )
        else:
        ...
        if d_ == "learner":
            ...
            if "reacher2_act" in env["learner"]["name"]:
                ...
                actor_loss = (
                    gen_weight * gen_loss + 100 * action_loss
                )  # + 0*eyeloss_state
            else:
                gen_weight = 0.001 * ph[d_]["train_disc"]
        ...
        if d_ == "expert":
            ...
            bc_grads_and_vars = bc_op.compute_gradients(
                loss=bc_loss, var_list=var_dict[d_]["actor_grad_vars"]
            )
        ...
        if d_ == "learner":
            ...
            auto_loss = tf.reduce_mean(
                tf.square(graph[d_]["inv_agent_state"] - auto_target_state)
            )
            auto_op = tf.train.AdamOptimizer(lr_auto * lr_decay_auto ** episodes)
            auto_grads_and_vars = auto_op.compute_gradients(
                loss=auto_loss, var_list=var_dict[d_]["auto_grad_vars"]
            )
        ...
        targets[d_]["train_model"] = {
            "model_train_op": model_train_op,
            "model_loss": model_loss,
        }

        if d_ == "expert":
            targets[d_]["bc"] = {"bc_train_op": bc_train_op, "bc_loss": bc_loss}
