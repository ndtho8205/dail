...
class Reacher3DOFPushEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        self.reset_called = False
        utils.EzPickle.__init__(self)
        mujoco_env.MujocoEnv.__init__(
            self,
            os.path.dirname(os.path.abspath(__file__)) + "/assets/reacher_3dof_push.xml",
            2,
        )
        self.viewer = None
    ...
    def _get_viewer(self):
        if self.viewer is None:
            ...
            self.viewer = mujoco_py.MjViewer(
                visible=True, init_width=size, init_height=size, go_fast=False
            )
            self.viewer.start()
