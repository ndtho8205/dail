...
class Reacher2DOFStoEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
    ...
    def reset_model(self):
        ...
        while True:
            ...
            self.goal = self.np_random.uniform(
                low=-max_reachable_len, high=max_reachable_len, size=2
            )
            ...
            if (
                ...
                and np.linalg.norm(self.goal) > min_reachable_len
            ):
                break
    ...
    def _get_obs(self):
        # randomly transition to the next state
        std = 0.05
        theta = np.random.normal(self.model.data.qpos.flat[:2], std)
        target = self.model.data.qpos.flat[2:]
        vel = np.random.normal(self.model.data.qvel.flat[:2], std)

        # manually set the simulation state with samples
        qpos = np.concatenate([theta, target], axis=0)
        qvel = np.zeros((self.model.nv,))
        qvel[:2] = vel
        self.set_state(qpos, qvel)

        return np.concatenate([np.cos(theta), np.sin(theta), target, vel])
    ...
    def _get_viewer(self):
        if self.viewer is None:
            self.viewer = mujoco_py.MjViewer(
                visible=True, init_width=200, init_height=200, go_fast=False
            )
            self.viewer.start()
