...
def get_initializer(name_list):
    ...
    for n_ in name_list:
        if n_ is None:
            ...
            inits.append(tf.initializers.truncated_normal(mean=0.0, stddev=0.1))
        else:
            print(f"Unrecognized initializer class: {n_}")
            exit(1)
...
def get_regularizer(name_list, scale=[0.001]):
    ...
    for i, n_ in enumerate(name_list):
        if n_ is None:
            ...
            regularizers.append(
                tf.contrib.layers.l1_l2_regularizer(scale_l1=scale[i], scale_l2=scale[i])
            )
        else:
            print(f"Unrecognized regularizer class: {n_}")
            exit(1)
...
def get_activation(name_list):
    ...
    for n_ in name_list:
        if n_ is None:
            ...
            print(f"Unrecognized activation class: {n_}")
            exit(1)
...
def scale_action(action, env):
    ...
    """
    scaled_action = env.action_space.low + tf.nn.sigmoid(action) * (
...
def scale_state(state, env):
    ...
    """
    scaled_state = env.observation_space.low + tf.nn.sigmoid(state) * (
...
def feedforward(
    in_node, is_training, params, scope, scale=False, scale_fn=None, scale_params=None
):
    """
