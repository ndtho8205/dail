...
def transfer_ddpg_graph_no_goal(env, ph, params):
    """
    ...
    for d_ in sorted(env.keys()):
        ...
        # Learner policy
            ...
            with tf.variable_scope("", reuse=tf.AUTO_REUSE):
                ...
                graph[d_]["mapped_state"] = feedforward(
                    in_node=ph[d_]["state"],
                    is_training=ph[d_]["is_training"],
        ...
        with tf.variable_scope("model", reuse=tf.AUTO_REUSE):
            sa_model = tf.concat([ph[d_]["state"], ph[d_]["action"]], axis=1)
            sa_raw = tf.concat([ph[d_]["state"], ph[d_]["raw_action"]], axis=1)
            ...
            graph[d_]["model_raw_next_state"] = feedforward(
                in_node=sa_raw,
                is_training=ph[d_]["is_training"],
                params=params[d_]["model"],
                scope=d_,
    ...
    with tf.variable_scope("model", reuse=True):
        sa = tf.concat([ph[d_]["state"], action], axis=1)
        sa_trans = tf.concat([graph[d_]["mapped_state_end"], trans_action], axis=1)
...
def get_transfer_ddpg_no_goal_vars(env):
    """
...
def get_transfer_ddpg_no_goal_targets(env, ph, graph, var_dict, params):
    """
    ...
    for d_ in env.keys():
        ...
        if d_ == "learner":
            ...
            actor_loss = 0 * gen_loss + action_loss  # + 0*eyeloss_state
