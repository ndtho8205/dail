...
def transfer_ddpg_graph_with_goal(env, ph, params):
    ...
    for d_ in sorted(env.keys()):
        ...
        if d_ == "learner":
            ...
            with tf.variable_scope("setsizemap", reuse=tf.AUTO_REUSE):
                ...
                set_q = feedforward(
                    ...
                    params=params[d_]["setsizemap"],
                    scope=d_,
                )

                next_set_q = feedforward(
                    in_node=ph[d_]["next_state"],
                    is_training=ph[d_]["is_training"],
                    params=params[d_]["setsizemap"],
                    scope=d_,
                )
                ...
                graph[d_]["set_size_sample"] = eps_greedy_sample(
                    set_logits, ph[d_]["epsilon"]
                )
                graph[d_]["next_set_size_sample"] = eps_greedy_sample(
                    next_set_logits, ph[d_]["epsilon"]
                )

                # Setsizemap qvalues: [batch, 1]
                mask = tf.one_hot(
                    ph[d_]["set_size"], depth=params["train"]["max_set_size"]
                )
                graph[d_]["qvalue_setsize"] = tf.reduce_sum(
                    set_q * mask, axis=1, keepdims=True
                )

            # Target network for setsizemap
            with tf.variable_scope("slow_setsizemap", reuse=False):
                # Q function for set sizes with an output size of max_set_size
                slow_set_q = feedforward(
                    in_node=ph[d_]["next_state"],
                    is_training=ph[d_]["is_training"],
                    params=params[d_]["setsizemap"],
                    scope=d_,
                ...
                graph[d_]["slow_set_size_sample"] = eps_greedy_sample(
                    slow_set_logits, ph[d_]["epsilon"]
                )  ## TODO: Replace sampling function

                # Target setsize qvalue for training setsizemap: [batch, 1]
                graph[d_]["slow_qvalue_setsize"] = tf.reduce_max(
                    slow_set_q, axis=1, keepdims=True
                )

        # ========= ACTOR ==========
        # Expert policy
        if d_ == "expert":
            graph[d_]["action"] = feedforward(
                in_node=ph[d_]["state"],
                is_training=ph[d_]["is_training"],
        ...
        # Learner policy
            ...
            with tf.variable_scope("", reuse=tf.AUTO_REUSE):
                ...
                agent_state = tf.concat(
                    [
                        ph[d_]["state"][:, : 2 * LEA_NJOINTS],
                        ph[d_]["state"][:, 2 * LEA_NJOINTS + 2 :],
                    ],
                    axis=1,
                )
                goal_state = (
                    ph[d_]["state"][:, 2 * LEA_NJOINTS : 2 * LEA_NJOINTS + 2] * GOAL_SCALE
                )

                graph[d_]["mapped_agent_state"] = feedforward(
                    in_node=agent_state,
                    is_training=ph[d_]["is_training"],
                    params=params[d_]["statemap"],
                    scope="actor/" + d_ + "/statemap",
                    scale=params["train"]["scale_state"],
                    scale_fn=scale_state,
                    scale_params=env[trans_d_]["env"],
                )

                graph[d_]["mapped_state"] = tf.concat(
                    [
                ...
                mask = tf.one_hot(
                    tf.squeeze(graph[d_]["set_size_sample"], axis=1),
                    depth=params["train"]["max_set_size"],
                )  # [batch, max_set_size]
                ...
                for t in range(1, params["train"]["max_set_size"]):
                    ...
                    next_state = feedforward(
                        in_node=sa,
                        is_training=ph[d_]["is_training"],
                        params=params[trans_d_]["model"],
                        scope="model/" + trans_d_,
                    )
                ...
                graph[d_]["premap_action"] = feedforward(
                    in_node=graph[d_]["mapped_state_end"],
                    is_training=ph[d_]["is_training"],
                    params=params[trans_d_]["actor"],
                ...
                sa_action = tf.concat(
                    [graph[d_]["premap_action"], ph[d_]["state"]], axis=1
                )

                graph[d_]["action"] = feedforward(
                    in_node=graph[d_]["premap_action"],
                    is_training=ph[d_]["is_training"],
        ...
        # Expert policy
        if d_ == "expert":
            graph[d_]["slow_target_action"] = feedforward(
                in_node=ph[d_]["next_state"],
                is_training=ph[d_]["is_training"],
                params=params[d_]["actor"],
                scope="slow_target_actor/" + d_,
                scale=True,
                scale_fn=scale_action,
                scale_params=env[d_]["env"],
            )
        ...
        # Learner policy
            ...
            with tf.variable_scope("", reuse=tf.AUTO_REUSE):
                ...
                next_agent_state = tf.concat(
                    [
                        ph[d_]["next_state"][:, : 2 * LEA_NJOINTS],
                        ph[d_]["next_state"][:, 2 * LEA_NJOINTS + 2 :],
                    ],
                    axis=1,
                )
                next_goal_state = (
                    ph[d_]["next_state"][:, 2 * LEA_NJOINTS : 2 * LEA_NJOINTS + 2]
                    * GOAL_SCALE
                )

                # Statemapping from learner to expert space
                mapped_agent_state = feedforward(
                    in_node=next_agent_state,
                    is_training=ph[d_]["is_training"],
                    params=params[d_]["statemap"],
                ...
                mask = tf.one_hot(
                    tf.squeeze(graph[d_]["next_set_size_sample"], axis=1),
                    depth=params["train"]["max_set_size"],
                )  # [batch, max_set_size]
                split_mask = tf.split(mask, params["train"]["max_set_size"], axis=1)
                assert len(split_mask) == params["train"]["max_set_size"]
                ...
                premap_action = feedforward(
                    in_node=mapped_state_end,
                    is_training=ph[d_]["is_training"],
                    params=params[trans_d_]["actor"],
        ...
        with tf.variable_scope("model", reuse=tf.AUTO_REUSE):
            ...
            model_agent_state = tf.concat(
                [
                    ph[d_]["state"][:, : 2 * njoints],
                    ph[d_]["state"][:, 2 * njoints + 2 :],
                ],
                axis=1,
            )
            ...
            graph[d_]["model_next_state"] = feedforward(
                in_node=sa_model,
                is_training=ph[d_]["is_training"],
            ...
            model_next_agent_state = feedforward(
                in_node=sa_raw,
                is_training=ph[d_]["is_training"],
                params=params[d_]["model"],
                scope=d_,
            ...
            graph[d_]["model_raw_next_state"] = tf.concat(
                [
        ...
        with tf.variable_scope("critic", reuse=tf.AUTO_REUSE):
            ...
            graph[d_]["qvalue_critic"] = feedforward(
                in_node=sa_critic,
                is_training=ph[d_]["is_training"],
            ...
            graph[d_]["qvalue_actor"] = feedforward(
                in_node=sa_actor,
                is_training=ph[d_]["is_training"],
                params=params[d_]["critic"],
                scope=d_,
        ...
        with tf.variable_scope("slow_target_critic", reuse=False):
            ...
            sa_target = tf.concat(
                [ph[d_]["next_state"], graph[d_]["slow_target_action"]], axis=1
            )
            graph[d_]["qvalue_target"] = tf.stop_gradient(
                feedforward(
                    in_node=sa_target,
                    is_training=ph[d_]["is_training"],
                    params=params[d_]["critic"],
    ...
    agent_trans_state = tf.concat(
        [
            graph[d_]["mapped_state_end"][:, : 2 * EXP_NJOINTS],
    ...
    with tf.variable_scope("model", reuse=True):
        ...
        # Next expert state
        trans_agent_next_state = feedforward(
            in_node=sa_trans,
            is_training=ph[d_]["is_training"],
            params=params[trans_d_]["model"],
            scope=trans_d_,
    ...
    with tf.variable_scope("actor", reuse=True):
        mapped_agent_next_state = feedforward(
            in_node=next_state,
            is_training=ph[d_]["is_training"],
            params=params[d_]["statemap"],
            scope=d_ + "/statemap",
    ...
    with tf.variable_scope("discriminator", reuse=tf.AUTO_REUSE):
        ...
        sas_real = tf.concat(
            [
                ph[trans_d_]["state"],
                ph[trans_d_]["raw_action"],
                graph[trans_d_]["model_raw_next_state"],
            ],
            axis=1,
        )
        ...
        graph[d_]["fake_prob"] = feedforward(
            in_node=sas_fake,
            is_training=ph[d_]["is_training"],
        ...
        graph[d_]["real_prob"] = feedforward(
            in_node=sas_real,
            is_training=ph[d_]["is_training"],
            params=params[d_]["discriminator"],
            scope=d_,
        )
        graph[d_]["mixed_prob"] = feedforward(
            in_node=sas_mixed,
            is_training=ph[d_]["is_training"],
            params=params[d_]["discriminator"],
            scope=d_,
...
def get_transfer_ddpg_with_goal_vars(env):
    ...
    for d_ in env.keys():
        ...
        if d_ == "expert":
            graph_vars[d_]["actor_grad_vars"] = tf.get_collection(
                tf.GraphKeys.GLOBAL_VARIABLES, scope="actor/" + d_
            )

            # Variables for target network updates
            graph_vars[d_]["all_actor_vars"] = tf.get_collection(
                tf.GraphKeys.GLOBAL_VARIABLES, scope="actor/" + d_
            )
            graph_vars[d_]["all_slow_actor_vars"] = tf.get_collection(
                tf.GraphKeys.GLOBAL_VARIABLES, scope="slow_target_actor/" + d_
            )

        else:
            # graph_vars[d_]['actor_grad_vars'] = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='actor/'+d_+'/statemap')
            graph_vars[d_]["actor_grad_vars"] = tf.get_collection(
                tf.GraphKeys.GLOBAL_VARIABLES, scope="actor/" + d_ + "/actionmap"
            )
            ...
            # Variables for target network updates
            graph_vars[d_]["all_actor_vars"] = tf.get_collection(
                tf.GraphKeys.GLOBAL_VARIABLES, scope="actor/" + d_ + "/statemap"
            )
            graph_vars[d_]["all_actor_vars"] += tf.get_collection(
                tf.GraphKeys.GLOBAL_VARIABLES, scope="actor/" + d_ + "/actionmap"
            )
            graph_vars[d_]["all_actor_vars"] += tf.get_collection(
                tf.GraphKeys.GLOBAL_VARIABLES, scope="actor/" + d_ + "/expert_pi"
            )
            ...
            graph_vars[d_]["setsizemap_grad_vars"] = tf.get_collection(
                tf.GraphKeys.GLOBAL_VARIABLES, scope="setsizemap/" + d_
            )
            graph_vars[d_]["all_setsizemap_vars"] = tf.get_collection(
                tf.GraphKeys.GLOBAL_VARIABLES, scope="setsizemap/" + d_
            )
            graph_vars[d_]["all_slow_setsizemap_vars"] = tf.get_collection(
                tf.GraphKeys.GLOBAL_VARIABLES, scope="slow_setsizemap/" + d_
            )

        # Critic, statemap, autoencoder grad vars
        graph_vars[d_]["critic_grad_vars"] = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope="critic/" + d_
        )
        graph_vars[d_]["all_critic_vars"] = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope="critic/" + d_
        )
        graph_vars[d_]["all_slow_critic_vars"] = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope="slow_target_critic/" + d_
        )

        # Statemap, autoencoder grad_vars
        graph_vars[d_]["statemap_grad_vars"] = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope="actor/" + d_ + "/statemap"
        )
        graph_vars[d_]["auto_grad_vars"] = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope="actor/" + d_ + "/statemap"
        )  ### TODO: consider changing this bothways

        # Dynamics model vars
        graph_vars[d_]["model_grad_vars"] = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope="model/" + d_
        )

        # Discriminator vars
        graph_vars[d_]["disc_grad_vars"] = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope="discriminator/" + d_
        )
        ...
        learner_save_vars = [
            var
            for var in all_vars
            if ("learner" in var.name and "expert_pi" not in var.name)
        ]
...
def get_transfer_ddpg_with_goal_targets(env, ph, graph, var_dict, params):
    ...
    for d_ in env.keys():
        ...
        if d_ == "learner":
            q_target = (
                tf.expand_dims(ph[d_]["disc_reward"], 1)
                + tf.expand_dims(ph[d_]["is_not_terminal"], 1)
                * gamma
                * graph[d_]["slow_qvalue_setsize"]
            )
            ...
            setsizemap_op = tf.train.AdamOptimizer(
                lr_setsizemap * lr_decay_setsizemap ** episodes
            )
            setsizemap_grads_and_vars = setsizemap_op.compute_gradients(
                loss=setsizemap_loss, var_list=var_dict[d_]["setsizemap_grad_vars"]
            )
            setsizemap_train_op = setsizemap_op.apply_gradients(
                grads_and_vars=setsizemap_grads_and_vars
            )

        else:
            setsizemap_loss = tf.constant(0.0)
            setsizemap_train_op = tf.constant(0.0)

        # Critic loss
        # 1-step temporal difference errors
        td_target = (
            tf.expand_dims(ph[d_]["reward"], 1)
            + tf.expand_dims(ph[d_]["is_not_terminal"], 1)
            * gamma
            * graph[d_]["qvalue_target"]
        )
        ...
        critic_grads_and_vars = critic_op.compute_gradients(
            loss=critic_loss, var_list=var_dict[d_]["critic_grad_vars"]
        )
        ...
        if d_ == "learner" and params[d_]["use_bc"]:
            action_loss = tf.reduce_mean(
                tf.square(graph[d_]["action"] - graph["expert"]["action"])
            )
        ...
        if d_ == "learner":
            ...
            for t in range(t_horizon):
                temp_diff = tf.square(
                    graph[d_]["multi_mapped_next_state"][t]
                    - graph[d_]["multi_trans_next_state"][t]
                ) * tf.expand_dims(ph[d_]["is_not_terminal"], 1)
            ...
            actor_loss = gen_loss + 0.0 * action_loss  # + 0*eyeloss_state
        ...
        if params["train"]["use_wgan"]:
            actor_op = tf.train.RMSPropOptimizer(
                0.1 * lr_actor * lr_decay_actor ** episodes
            )
        ...
        actor_grads_and_vars = actor_op.compute_gradients(
            loss=actor_loss, var_list=var_dict[d_]["actor_grad_vars"]
        )
        ...
        njoints = EXP_NJOINTS if d_ == "expert" else LEA_NJOINTS
        model_target_state = tf.concat(
            [
                ph[d_]["next_state"][:, : 2 * njoints],
                ph[d_]["next_state"][:, 2 * njoints + 2 :],
            ],
            axis=1,
        )
        model_diff = tf.square(
            graph[d_]["model_next_state"] - model_target_state
        ) * tf.expand_dims(ph[d_]["is_not_terminal"], 1)
        model_loss = tf.reduce_mean(model_diff)
        model_op = tf.train.AdamOptimizer(lr_model * lr_decay_model ** episodes)
        model_grads_and_vars = model_op.compute_gradients(
            loss=model_loss, var_list=var_dict[d_]["model_grad_vars"]
        )
        ...
        if d_ == "learner":
            ...
            if params["train"]["use_wgan"]:
                disc_loss = -tf.reduce_mean(graph[d_]["real_prob"]) + tf.reduce_mean(
                    graph[d_]["fake_prob"]
                )
                disc_op = tf.train.RMSPropOptimizer(
                    0.1 * lr_disc * lr_decay_disc ** episodes
                )
                disc_grads_and_vars = disc_op.compute_gradients(
                    loss=disc_loss * ph[d_]["train_disc"],
                    var_list=var_dict[d_]["disc_grad_vars"],
                )
                disc_train_op = [
                    disc_op.apply_gradients(grads_and_vars=disc_grads_and_vars)
                ]
                disc_weight_clip = [
                    var.assign(tf.clip_by_value(var, -0.1, 0.1))
                    for var in var_dict[d_]["disc_grad_vars"]
                ]
                ...
                disc_train_op = disc_op.apply_gradients(
                    grads_and_vars=disc_grads_and_vars
                )
                ...
                fake_loss = tf.nn.sigmoid_cross_entropy_with_logits(
                    logits=graph[d_]["fake_prob"],
                    labels=tf.zeros_like(graph[d_]["fake_prob"]),
                )
                fake_loss = tf.reduce_mean(fake_loss)
                real_loss = tf.nn.sigmoid_cross_entropy_with_logits(
                    logits=graph[d_]["real_prob"],
                    labels=tf.ones_like(graph[d_]["real_prob"]),
                )
        ...
        assert len(var_dict[d_]["all_slow_actor_vars"]) == len(
            var_dict[d_]["all_actor_vars"]
        )
        assert len(var_dict[d_]["all_slow_critic_vars"]) == len(
            var_dict[d_]["all_critic_vars"]
        )
        ...
        for i, slow_target_actor_var in enumerate(var_dict[d_]["all_slow_actor_vars"]):
            ...
            update_slow_target_actor_op = slow_target_actor_var.assign(
                tau * actor_var + (1 - tau) * slow_target_actor_var
            )
        ...
        for i, slow_target_var in enumerate(var_dict[d_]["all_slow_critic_vars"]):
            ...
            update_slow_target_critic_op = slow_target_var.assign(
                tau * critic_var + (1 - tau) * slow_target_var
            )
        ...
        if d_ == "learner":
            assert len(var_dict[d_]["all_slow_setsizemap_vars"]) == len(
                var_dict[d_]["all_setsizemap_vars"]
            )
            for i, slow_target_setsize_var in enumerate(
                var_dict[d_]["all_slow_setsizemap_vars"]
            ):
                setsize_var = var_dict[d_]["all_setsizemap_vars"][i]
                update_slow_target_setsize_op = slow_target_setsize_var.assign(
                    tau * setsize_var + (1 - tau) * slow_target_setsize_var
                )
                targets[d_]["update"].append(update_slow_target_setsize_op)

        targets[d_]["update"] = tf.group(
            *targets[d_]["update"], name="update_slow_targets"
        )
