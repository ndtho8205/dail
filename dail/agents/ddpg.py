...
class DDPGAgent:
    def __init__(
        ...
        Path(self.save_expert_dir).mkdir(parents=True, exist_ok=True)
        Path(self.save_learner_dir).mkdir(parents=True, exist_ok=True)
        Path(self.save_dataset_dir).mkdir(parents=True, exist_ok=True)
    ...
    def invoke_killer(self, render_option):
        epsilon = {"expert": 1.0, "learner": 1.0}
        ...
        if self.killer.kill_now:
            ...
            if visual_option == "y":
                if render_option == "expert_policy":
                    ...
                    self.render_statemap(
                        {"epsilon": epsilon}, num_rollout=10, save_dir="learner_policy"
                    )
                    ...
                    print(f"ERROR: unrecognized render option '{render_option}'")

            save_option = input("Save current model (y/[n])?")
            if save_option == "y":
                if "expert" in render_option:
                    self.saver_expert.save(
                        self.sess, self.save_expert_dir + "/expert.ckpt"
                    )
                elif "learner" in render_option:
                    self.saver_learner.save(
                        self.sess, self.save_learner_dir + "/learner.ckpt"
                    )
                else:
                    print(f"ERROR: unrecognized render option '{render_option}'")
    ...
    def train_expert(self, from_ckpt):
        ...
        expert_readouts = {"total_steps": 0, "total_reward": 0, "steps_in_ep": 0}

        learner_readouts = {"total_steps": 0, "total_reward": 0, "steps_in_ep": 0}
        ...
        for ep in range(self.params["train"]["num_episodes"]):
            ...
            noise_scale = (
                ...
                * (self.params["train"]["noise_decay"] ** ep)
                * action_range
            )
            ...
            for t in range(self.params["train"]["max_steps_ep"]):
                ...
                (raw_action,) = self.sess.run(
                    self.graph[d_]["action"],
                    feed_dict={
                        self.ph[d_]["state"]: obs[None],
                ...
                if (
                    readouts[d_]["total_steps"] % self.params["train"]["train_every"] == 0
                    and self.replay_memory["expert"].len()
                    >= self.params["train"]["batchsize"]
                    and not stop_train[d_]
                ):

                    # grab N (s,a,r,s') tuples from replay memory
                    minibatch = self.replay_memory[d_].sample_from_memory(
                        batchsize=self.params["train"]["batchsize"]
                    )

                    # update the critic and actor params using mean-square value error
                    # and deterministic policy gradient, respectively
                    feed_dict = {
                        self.ph[d_]["state"]: np.asarray([elem[0] for elem in minibatch]),
                        self.ph[d_]["action"]: np.asarray(
                            [elem[1] for elem in minibatch]
                        ),
                        self.ph[d_]["reward"]: np.asarray(
                            [elem[2] for elem in minibatch]
                        ),
                        self.ph[d_]["next_state"]: np.asarray(
                            [elem[3] for elem in minibatch]
                        ),
                        self.ph[d_]["is_not_terminal"]: np.asarray(
                    ...
                    fetches = self.sess.run(
                        self.targets[d_]["train_rl"], feed_dict=feed_dict
                    )
            ...
            save_reward = tf.Summary.Value(
                tag="Reward", simple_value=readouts["expert"]["total_reward"]
            )
            self.writer.add_summary(
                tf.Summary(value=[save_reward]), readouts["expert"]["total_steps"]
            )
    ...
    def create_alignment_taskset(self):
        ...
        self.learner_save_vars = [
            var for var in self.learner_save_vars if "invmap" not in var.name
        ]
        self.saver_learner = tf.train.Saver(var_list=self.learner_save_vars)
        self.saver_learner.restore(self.sess, self.load_learner_dir + "/learner.ckpt")

        # Copy the expert policy into new variable
        expert_pi_vars = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope="actor/expert"
        )
        learner_actor_pi_vars = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope="actor/learner/expert_pi"
        )
        learner_slow_actor_pi_vars = tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES, scope="slow_target_actor/learner/expert_pi"
        )
    ...
    def gama(self, from_ckpt):
        ...
        if from_ckpt:
            self.saver_learner.restore(self.sess, self.load_learner_dir + "/learner.ckpt")
            self.params["train"]["initial_noise_scale"] = 0.0
        ...
        print(f"Loading data from: {self.load_dataset_dir}")
        ...
        for idx in range(100000):
            ...
            # grab N (s,a,r,s') tuples from replay memory
            minibatch_l = self.replay_memory["learner"].sample_from_memory(
                batchsize=self.params["train"]["batchsize"]
            )

            # update the critic and actor params using mean-square value error and deterministic policy gradient, respectively
            feed_dict = {
                self.ph[d_]["state"]: np.asarray([elem[0] for elem in minibatch_l]),
                ...
                self.ph[d_]["is_not_terminal"]: np.asarray(
                    [elem[4] for elem in minibatch_l]
                ),
                self.ph[d_]["is_training"]: True,
                self.ph[d_]["train_disc"]: 0,
            }
            ...
            if idx % 100 == 0:
                print(
                    "Step {} | Learner dynamics loss: {:.2e}".format(
                        idx, np.mean(dynamics_loss)
                    ),
                    end="\r",
                )
                dynamics_loss = []
        ...
        while True:
            ...
            for idx in tqdm(range(500)):
                ...
                feed_dict = {
                    self.ph[d_]["state"]: np.asarray([elem[0] for elem in minibatch_l]),
                    self.ph[d_]["action"]: np.asarray([elem[1] for elem in minibatch_l]),
                    self.ph[d_]["raw_action"]: np.asarray(
                        [elem[5] for elem in minibatch_l]
                    ),
                    self.ph[d_]["reward"]: np.asarray([elem[2] for elem in minibatch_l]),
                    self.ph[d_]["next_state"]: np.asarray(
                        [elem[3] for elem in minibatch_l]
                    ),
                    self.ph[d_]["is_not_terminal"]: np.asarray(
                        [elem[4] for elem in minibatch_l]
                    ),
                    self.ph[d_]["is_training"]: True,
                    self.ph[d_]["train_disc"]: 0.0,
                    self.ph[d_]["disc_reward"]: np.asarray(
                        [elem[8] for elem in minibatch_l]
                    ),
                }

                if step % 1 == 0:
                    minibatch_e = self.replay_memory["expert"].sample_from_memory(
                        batchsize=self.params["train"]["batchsize"]
                    )
                    feed_dict_expert = {
                        self.ph["expert"]["state"]: np.asarray(
                            [elem[0] for elem in minibatch_e]
                        ),
                        self.ph["expert"]["action"]: np.asarray(
                            [elem[1] for elem in minibatch_e]
                        ),
                        self.ph["expert"]["next_state"]: np.asarray(
                            [elem[3] for elem in minibatch_e]
                        ),
                        self.ph["expert"]["raw_action"]: np.asarray(
                            [elem[5] for elem in minibatch_e]
                        ),
                        self.ph["expert"]["is_training"]: True,
                    }

                    feed_dict.update({self.ph[d_]["train_disc"]: 1.0})
                    ...
                    targets = {
                        k: v
                        for k, v in self.targets[d_]["train_gama"].items()
                        if "disc" not in k
                    }
            ...
            readouts[d_]["total_reward"] = self.render_statemap(
                {"epsilon": epsilon},
                num_rollout=25,
                save_video=False,
            ...
            print(f"Best reward: {best_reward:.2f}")
            print(f"Best gama reward: {best_gama_reward:.2f}")
            print(f"Best gama loss: {best_gama_loss:.6f}")
            print("______________________________")

            # Save to tensorboard
            save_reward = tf.Summary.Value(
                tag="Best Reward", simple_value=readouts["learner"]["total_reward"]
            )
            save_gama_reward = tf.Summary.Value(
                tag="Best GAMA Reward", simple_value=best_gama_reward
            )
            gama_loss = tf.Summary.Value(
                tag="GAMA Loss", simple_value=np.mean(readouts["learner"]["gama_loss"])
            )
            gen_loss = tf.Summary.Value(
                tag="Generator Loss",
                simple_value=np.mean(readouts["learner"]["gen_loss"]),
            )
    ...
    def zeroshot(self):
        ...
        vid_name = (
            self.args.doc + f"_{self.load_learner_dir[-1]}"
            if self.args.doc
            else self.load_learner_dir[-6:]
        )
        self.render_statemap(
            {"epsilon": epsilon}, num_rollout=5, save_dir=vid_name, save_video=True
        )
        print("Done!")
    ...
    def bc(self, from_ckpt=False, num_demo=-1):
        ...
        print(f"state data shape: {exp_o.shape}")
        print(f"action data shape: {exp_a.shape}")
        ...
        for epoch in range(num_epochs):
            ...
            for i in tqdm(range(batches_per_epoch)):
                ...
                fetches = self.sess.run(
                    self.targets["expert"]["bc"],
                    feed_dict={
                        self.ph["expert"]["state"]: batch_o,
            ...
            print(f"Epoch {epoch} | bc loss: {epoch_loss}, rl loss: {avg_reward}")
    ...
    def rollout_expert(self):
        ...
        vid_name = (
            self.args.doc + f"_{self.load_expert_dir[-1]}"
            if self.args.doc
            else self.load_expert_dir[-6:]
        )
        render_policy(
            self.sess,
            self.graph,
    ...
    def render_statemap(
        self, loop_vars, save_video=True, num_rollout=20, save_dir="learner_policy"
    ):
        ...
        for idx in range(num_rollout):
            ...
            while not done:
                mapped_state_raw, trans_act, act = self.sess.run(
                    [
                        self.graph["learner"]["mapped_state"],
                        self.graph["learner"]["premap_action"],
                        self.graph["learner"]["action"],
                    ],
                    feed_dict={
                        self.ph["learner"]["state"]: obs[None],
    ...
    def render_dynamics(
        self, loop_vars, domain, save_video=True, num_rollout=20, save_dir="dynamics"
    ):
        ...
        for idx in range(num_rollout):
            ...
            while ep_step < 60:
                ...
                next_obs = self.sess.run(
                    self.graph[d_]["model_next_state"],
                    feed_dict={
                        self.ph[d_]["state"]: obs[None],
