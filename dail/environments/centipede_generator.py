    ...
    for i_leg in range(num_legs):
        ...
        if is_right_leg > 0:
            xml_content += (
                ...
                + f' joint="righthip_{i_leg}" gear="150"/>\n'
            )
        else:
            xml_content += (
                '    <motor ctrllimited="true" ctrlrange="-1.0 1.0"'
                + f' joint="lefthip_{i_leg}" gear="150"/>\n'
            )
        xml_content += (
            '    <motor ctrllimited="true" ctrlrange="-1.0 1.0"'
            + f' joint="ankle_{i_leg}" gear="150"/>\n'
        )
        if i_leg % 2 == 1 and i_leg > 1:
            # add the body joint
            xml_content += (
                '    <motor ctrllimited="true" ctrlrange="-1.0 1.0"'
                + f' joint="body_{int(np.floor(i_leg / 2))}" gear="100"/>\n'
            )
            xml_content += (
                '    <motor ctrllimited="true" ctrlrange="-1.0 1.0"'
                + f' joint="bodyupdown_{int(np.floor(i_leg / 2))}" gear="100"/>\n'
            )
