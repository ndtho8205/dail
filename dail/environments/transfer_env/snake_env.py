...
class SnakeEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, pod_number=3, is_crippled=False):
        ...
        if is_crippled:
            xml_name = "CrippledSnake" + self.get_env_num_str(pod_number) + ".xml"
        else:
        ...
        # xml_path = os.path.join(os.path.join(os.getcwd()+'/..', 'assets', xml_name))
        xml_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)) + "/..", "assets", xml_name
        )
    ...
    def set_state_from_obs(self, obs):
        # self.set_state(np.concatenate([np.array([0., 0.]), obs[:3]]), obs[3:])
        """
        if self.num_body == 3:
            self.set_state(np.concatenate([self.model.data.qpos.flat[:2], obs[:3]]), obs[3:])
        elif self.num_body == 4:
            self.set_state(np.concatenate([self.model.data.qpos.flat[:2], obs[:4]]), obs[4:])
        """
        n = self.num_body
        self.set_state(np.concatenate([self.model.data.qpos.flat[:2], obs[:n]]), obs[n:])
